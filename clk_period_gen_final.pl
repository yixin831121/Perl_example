...
# user setting
	...
	# output minimum period report and sdc variable file
	my $out_csv = "./min_period_list.csv";
	my $sdc_var = "./CLK_PERIOD.sdcVar.sdc";

	# test mode : random corner input, you can moditfy the $random_corner_number to change the number of input corner
	my $test_mode = 1;
	my $random_corner_number = 4;

	# full case of sdf_cond : set 0 if you just want to get minimum period when sram enable. If set 1, this program will generate minimum period report for different sdf condition. 
...
foreach my $group_key (sort {$a <=> $b} keys %group)
{
	...
	foreach my $sdf_cond_key (keys %{$group{$group_key} -> sdf_cond})
	{
		...
		print "SDF condition," . $sdf_cond_key . "\n" if($full_case_sdf_cond);
...
print "\n[Info]  Minimum period report is generated ...\n";
...
#	foreach my $corner (keys %{$sdcVar{$sdf_cond} -> sdf_cond_corner})
#	{
#		print "\tcorner = $corner\n";
...
#			print "\t\tgroup$group : $clk_period\n";
#		}
#	}
#}

if(!($full_case_sdf_cond == 1))
{	
